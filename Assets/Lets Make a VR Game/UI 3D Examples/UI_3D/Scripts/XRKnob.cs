...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRKnob : XRBaseInteractable
    {
        ...
        public bool XangleRotation, YangleRotation, ZangleRotation;
        /// <summary>
        ...
        void UpdateRotation(bool freshCheck = false)
        {
            ...
            if (XangleRotation)
            {
                localOffset.x = 0.0f;
                localUp.x = 0.0f;
                localY = Math.Abs(localForward.x);
            }
            else if (YangleRotation)
            {
                localOffset.y = 0.0f;
                localUp.y = 0.0f;
                localY = Math.Abs(localForward.y);
            }   
            else
            {
                localOffset.z = 0.0f;
                localUp.z = 0.0f;
                localY = Math.Abs(localForward.z);
            }
